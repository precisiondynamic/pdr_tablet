...
(async () => {
    ...
    await launch(APP, undefined, 1500);
    check('home hero with chips + heat + 7-day earnings chart', await (await f()).$('.hero .chip') && await (await f()).$('.hero .heat') && await (await f()).$('.card-earn .chart-bar'));
    check('comms unread shown in nav', (await text('[data-nav=comms] .nav-count')) === '2');
    await (await f()).click('[data-nav=map]'); await wait(300);
    check('city map shows one coloured zone per contract', (await (await f()).$$('.map-hit')).length === 3);
